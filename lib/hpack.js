...
Context.prototype.compress = function compress(headers) {
  ...
  for(var name in headers) {
    ...
    if (result.matched_entry) {
      ...
      entry.reference = true;
      entry.emitted = true;
      var evicted = this._header_table.unshift(entry);

      // Encode evicted entries to indexed rep
      for (var i=0, len=evicted.length; i<len; i++) {
        if (evicted[i].entry.common) {
          var rep = this._encodeHeader({
            type:  REP_INDEXED,
            index: evicted[i].index
          });
          reps.push(rep);
          reps.push(rep);
        }
