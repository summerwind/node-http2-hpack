...
HeaderTable.prototype.limit = function limit(limit) {
  var removed = [];
...
HeaderTable.prototype.clear = function clear() {
  this._entries = this._entries.splice(0, this.length);
  this.length = 0;
  this.size = 0;
};
...
HeaderTable.prototype._evict = function _evict(entry) {
  ...
  var new_limit = this.size + entry.size;
  while (new_limit > this.limit) {
    removed.push(this.pop());
  }
...
// Huffman Table Class
// ========================================================
var HuffmanTable = function(type) {
  switch (type) {
    case 'request':
...
var Context = function(options) {
  ...
  this._header_table  = new HeaderTable(options.limit);
...
Context.prototype.compress = function compress(headers) {
  ...
  for(var name in headers) {
    ...
    if (result.matched_entry) {
      ...
      entry.reference = true;
      entry.emitted = true;
      var unshift_result = this._header_table.unshift(entry);

      // Encode removed entries to indexed rep
      for (var i=0, len=unshift_result.removed.length; i<len; i++) {
        if (unshift_result.removed[i].common) {
...
Context.prototype._dump = function _dump() {
  ...
  for (var i=0; i<this._header_table.length; i++) {
    tmp.push(this._header_table.get(i));
  }
  this._debug('Header Table', tmp);
};
