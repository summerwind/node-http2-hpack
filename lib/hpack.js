...
HeaderTable.prototype._evict = function _evict(entry) {
  ...
  while ((this.size + entry.size) > this.limit) {
    ...
    removed.push({ index: index, entry: this.pop() });
  }
  ...
  for(var name in headers) {
    ...
    if (result.matched_entry) {
      ...
      for (var i=0, len=unshift_result.removed.length; i<len; i++) {
        this._debugLog('Evicted entry', unshift_result.removed[i].entry);
        if (unshift_result.removed[i].entry.common) {
          var rep = this._encodeHeader({
            type:  REP_INDEXED,
            index: unshift_result.removed[i].index
          });
          reps.push(rep);
          reps.push(rep);
        }
...
Context.prototype._dump = function _dump() {
  ...
  this._debugLog(label, '--------------------');
  this._debugLog(label, reference_set);
};
